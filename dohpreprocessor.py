...
class PreprocessorDoH():
    ...
    def preprocess(self, IPFlowsDataFrame, nan_values=False):
        return self._process(IPFlowsDataFrame, nan_values)

    def _time2msec(self, timestamp) -> float:
        return timestamp.total_seconds() * 1000

    def _time_diff(self, t1, t2) -> float:
        return self._time2msec(datetime.strptime(t2, "%Y-%m-%dT%H:%M:%S.%f") - datetime.strptime(t1, "%Y-%m-%dT%H:%M:%S.%f"))
